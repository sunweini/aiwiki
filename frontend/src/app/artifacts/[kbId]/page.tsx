...
export default async function ArtifactViewerPage({ params }: { params: { kbId: string } }) {
  ...
  const graphArtifact = artifacts.find((a) => a.artifact_type === "html") ?? artifacts.find((a) => a.artifact_type === "graph");
    ...
    <main style={{ maxWidth: 1280, margin: "0 auto", padding: "2.5rem 3rem" }}>
      <header style={{ borderBottom: "1px solid var(--line)", paddingBottom: "1.25rem", marginBottom: "2rem" }}>
        ...
        <p style={{ margin: "0.6rem 0 0", color: "var(--muted)" }}>
          发布{" "}
          <code style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}>{releaseId ?? "无"}</code>
        </p>
      ...
      {graphArtifact ? (
        <section style={{ marginBottom: "2.5rem" }}>
          <h2 style={{ margin: "0 0 0.75rem" }}>知识图谱</h2>
        ...
        <section style={{ border: "1px solid var(--line)", padding: "1.25rem", background: "rgba(255,255,255,0.28)", marginBottom: "2.5rem" }}>
          <p style={{ color: "var(--muted)" }}>此知识库找不到图谱产物。</p>
        </section>
      )}

      <section style={{ marginBottom: "2.5rem" }}>
        <h2 style={{ margin: "0 0 0.75rem" }}>产物清单</h2>
        <div style={{ display: "grid", gap: "0.5rem" }}>
          {artifacts.length === 0 ? (
            <p style={{ color: "var(--muted)" }}>暂无已注册产物。</p>
          ) : (
            artifacts.map((a) => (
              <article
                key={a.id}
              ...
              >
                ...
                <Link href={`/knowledge-bases/${kbId}`} style={{ fontSize: "0.85rem" }}>知识库档案 →</Link>
              </article>
            ))
          )}
        </div>
      ...
      {vaultArtifacts.length > 0 ? (
        <section>
          <h2 style={{ margin: "0 0 0.75rem" }}>知识库索引</h2>
