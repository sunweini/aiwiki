...
## GraphifyRunner Pipeline (16 阶段)

```
resolve_job_context → materialize_sources → normalize_inputs
    → extract → sanitize → build → validate → cluster
    → analyze → report → export_obsidian → export_wiki
    → export_visual → enhance_obsidian → verify_query
...
| 2 | `materialize_sources` | `SourceMaterializer` | source 列表 | `source-materials/` 目录 | 是 |
...
| 14 | `verify_query` | `GraphifyQueryAdapter` | graph.json | BFS 查询验证 | 否 |
...
- **阻塞失败** (stage 2-5, 15)：返回 `graph_ready=false`，不注册 release
- **降级** (stage 6-14)：记录 warning，流水线继续
...
```
用户 → POST /api/knowledge-bases/{kb_id}/builds
  │
  ├─ BuildService.create_build_job()    → BuildJob (status=pending)
  ├─ BuildService.enqueue_build()       → GraphifyRunner.run()
  │   ├─ SourceMaterializer.materialize()   → 文件复制
  │   ├─ graphify.extract()                 → LLM 提取 (DeepSeek/OpenAI/...)
...
| 决策 | 原因 |
|------|------|
| Build job 同步执行 | V1 简化，V2 改用后台任务队列 |
